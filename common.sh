...
function parse_settings() {
	...
	if [[ $PACKAGES_REPO =~ (default|DEFAULT|Default) || -z $PACKAGES_REPO ]]; then
		PACKAGES_REPO="libntdll/openwrt-packages"
	fi
...
function modify_config() {
	...
	# 创建并初始化文件
	rm -rf "$CONFFLICTIONS" && touch "$CONFFLICTIONS" && chmod +x "$CONFFLICTIONS"
...
function compile_info() {
	...
	if [[ "$FIRMWARE_TYPE" == "lxc" ]]; then
		...
		__green_color "wget https://mirror.ghproxy.com/https://raw.githubusercontent.com/libntdll/pve/main/openwrt.lxc.sh -O /usr/bin/openwrt && chmod +x /usr/bin/openwrt"
